...
class lspeed:
  def __init__(self):
   ...
   # global current_time
    ...
    #global last_time
    #global l_encv
    rospy.Subscriber('/l_enc', Int16, self.enc_to_speed, queue_size = 10)
    self.l_encspeed = rospy.Publisher('/l_speed', Float64, queue_size=10)
  ...
  def run(self):
    while not rospy.is_shutdown():
      #enc_msg = Float64()
      #if ((self.enc_msg.data - self.l_encv.data) == 0)
      self.lock.acquire()
      self.enc_msg.data = self.l_encv.data
      self.l_encspeed.publish(self.enc_msg)
      self.lock.release()
      self.rate.sleep()
